...
使用方式：
  python react_manual.py                       # 进入多轮对话模式
  python react_manual.py --question "五粮液2023年净利润是多少？"
...
def run(question: str, max_steps: int = 10, memory: SessionMemory | None = None, verbose: bool = True) -> Generator[dict, None, None]:
    """
...
def run_and_print(question: str, memory: SessionMemory | None = None, max_steps: int = 10) -> SessionMemory:
    if memory is None:
    ...
    for step_data in run(question, memory=memory, max_steps=max_steps):
        step_count += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--question", default=None, help="单次问题；省略则进入多轮对话模式")
    ...
    if args.question:
        ...
        print("进入多轮对话模式，输入 'exit' 或 'quit' 退出")
        memory = SessionMemory()
        while True:
            try:
                question = input("> ")
            ...
            if not question or question.lower() in ("exit", "quit"):
                break
            memory = run_and_print(question, memory=memory, max_steps=args.max_steps)
