...
def build_label_schema() -> tuple[list[str], dict[str, int], dict[int, str]]:
    ...
    与 data/peoples_daily/label_names.json 一致的顺序：
...
def _get_word_ids(encoding, tokenizer) -> list:
    """兼容 fast / slow tokenizer 的 word_ids 获取。

    fast tokenizer 直接调用 encoding.word_ids()；
    slow tokenizer（BertTokenizer 默认）没有该方法，改为手动对齐：
      用 tokenizer.convert_ids_to_tokens 还原子词序列，
      普通 token 递增 word_idx，'##' 开头的子词沿用当前 word_idx。
    """
    ...
    for token in sub_tokens:
        if token in special_tokens:
            ...
            word_ids.append(word_idx)  # 子词延续，属于当前 word
        else:
            if word_idx is None:
                word_idx = 0
            else:
...
class PeoplesDailyDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        word_ids = _get_word_ids(encoding, self.tokenizer)
        aligned_labels = []
        prev_word_id = None
        for wid in word_ids:
            if wid is None:
