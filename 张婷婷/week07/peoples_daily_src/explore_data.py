"""
人民日报 peoples_daily 数据集探索与可视化

与原 cluener 项目的区别：
  - 数据格式：token + BIO 标签（ner_tags），而非 span 格式（text + label）
  - 从 BIO 序列还原实体（B-X 开头，I-X 延续），而非直接读 span
  - 实体类型：PER / ORG / LOC 三类

教学重点：
  1. BIO 标注格式的统计方法（从标签序列还原实体 span）
...
def extract_entities(tokens: list[str], ner_tags: list[str]) -> list[tuple[str, str]]:
    """从 BIO 标签序列还原实体，返回 [(实体类型, 实体文本), ...]。

    BIO 规则：B-X 开启新实体，I-X 延续当前实体，O 不属于任何实体。
    """
    entities = []
    i = 0
    n = len(tokens)
    while i < n:
        ...
        if tag.startswith("B-"):
            ...
            while j < n and ner_tags[j] == f"I-{etype}":
                j += 1
            entities.append((etype, "".join(tokens[i:j])))
            i = j
        else:
            i += 1
    return entities
...
def print_summary(stats_train: dict, stats_val: dict):
    print("=" * 70)
    print("人民日报 peoples_daily 数据集统计摘要")
    print("=" * 70)
...
def plot_entity_distribution(stats_train: dict):
    ...
    ax.set_title("人民日报 各类实体频次分布（训练集）", fontsize=14)
    ax.set_ylabel("实体数量")
    ax.set_xlabel("实体类型")
    plt.tight_layout()
...
def parse_args():
    parser = argparse.ArgumentParser(description="探索人民日报 peoples_daily 数据集")
