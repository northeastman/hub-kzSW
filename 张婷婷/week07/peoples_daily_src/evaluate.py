"""
在测试集上评估 BERT NER 模型，并统计非法序列（人民日报 peoples_daily 数据集）

与原 cluener 项目的区别：
  - 数据来源：data/peoples_daily（token + BIO 标签），而非 data/cluener
  - 标签体系：PER / ORG / LOC 三类实体
  - 人民日报 test 集标签公开，可直接评估 test 集
...
def parse_args():
    parser = argparse.ArgumentParser(description="评估 BERT NER 模型（人民日报）")
    parser.add_argument("--use_crf", action="store_true")
    parser.add_argument("--bert_path", type=Path, default=BERT_PATH)
    parser.add_argument("--batch_size", type=int, default=32)
    parser.add_argument("--split", choices=["validation", "test"], default="validation",
                        help="人民日报 test 集标签已公开，可自由选择")
