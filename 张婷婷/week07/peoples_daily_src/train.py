"""
BERT NER 训练脚本（人民日报 peoples_daily 数据集）

与原 cluener 项目的区别：
  - 数据来源：data/peoples_daily（token + BIO 标签），而非 data/cluener（span 格式）
  - 标签体系：PER / ORG / LOC 三类实体，共 7 个 BIO 标签
  - 其余训练流程（分层学习率、warmup、CRF 对比等）与 cluener 项目一致
...
def parse_args():
    parser = argparse.ArgumentParser(description="训练 BERT NER 模型（人民日报）")
