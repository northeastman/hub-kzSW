...
def main():
    ...
    print(f"{CYAN}技能系统已初始化，发现 {len(skill_registry._registry)} 个技能{RESET}")
    print(f"{DIM}输入 /skills 查看可用技能{RESET}\n")
    ...
    while True:
        ...
        if user_input == "/skills":
            print(f"\n{CYAN}=== 可用技能 ==={RESET}")
            for skill in skill_registry.list_available_skills():
                status = f"{GREEN}[已加载]{RESET}" if skill["loaded"] else f"{DIM}[未加载]{RESET}"
                print(f"  {status} {skill['display_name']} ({skill['name']})")
                print(f"    {DIM}{skill['description']}{RESET}")
                if skill["triggers"]:
                    print(f"    {DIM}触发词: {', '.join(skill['triggers'])}{RESET}")
                print()
            stats = skill_registry.get_skill_stats()
            print(f"{DIM}总计: {stats['loaded_skills']}/{stats['total_skills']} 已加载{RESET}\n")
            continue

        # 尝试匹配并执行技能
        matched_skill = skill_registry.match_skill_by_query(user_input)
        if matched_skill:
            metadata = skill_registry._registry[matched_skill]
            print(f"{YELLOW}[技能匹配] {metadata.display_name}{RESET}")
            
            # 构建执行上下文
            context = {"query": user_input}
            
            # 执行技能
            result = skill_registry.execute_skill(matched_skill, context)
            
            if result.success:
                print(f"{GREEN}[技能输出] {result.output.get('message', str(result.output))}{RESET}")
                print(f"{DIM}执行时间: {result.execution_time:.3f}s{RESET}\n")
            else:
                print(f"{YELLOW}[技能错误] {result.error}{RESET}\n")
