...
使用方式：
  ...
  python rag_pipeline.py --query "电动自行车最高设计车速"
  python rag_pipeline.py --query "..." --doc GB17761-2024 --doc-type 标准
...
SYSTEM_PROMPT = """你是一个专业的标准法规问答助手，专门回答关于国家标准与法律法规的问题。

回答规则：
1. 只根据【参考资料】中的内容回答，不得引用或编造资料外的内容
2. 若参考资料不足以支撑回答，直接说"根据提供的资料无法回答此问题"
3. 引用具体条款或数值时，在句末标注来源编号，如：整车质量不超过55kg[1]
4. 涉及条款号、限值、参数时必须精确，不得模糊或臆断
5. 回答简洁，重点突出"""
...
class VectorStore:
    def __init__(self, client: OpenAI):
        ...
        import numpy as np
        self.client    = client
        # FAISS 的 C++ 读文件无法处理含中文的路径，改为 Python 读字节 + 反序列化
        ...
        messages=[
            {
                "role": "system",
                "content": (
                    "你是检索查询优化专家。将用户的问题改写为更适合从国家标准/法律法规中检索信息的精确查询语句。"
                    "保留关键实体（标准号、条款号、术语指标），扩展相关关键词，不要超过50字。"
                    "直接输出改写后的查询语句，不要解释。"
                ),
            },
...
class RAGPipeline:
    ...
    def query(
        ...
        if top_score < SCORE_THRESHOLD and filter_meta is None:
            return {
                "answer": "根据标准/法规知识库未能找到与该问题相关的内容，建议直接查阅原始标准或法律条文。",
...
def main():
    parser = argparse.ArgumentParser(description="标准/法律 RAG 问答（原生版）")
    parser.add_argument("--query",         type=str,  default=None)
    parser.add_argument("--doc",           type=str,  default=None, help="文档ID，如 GB17761-2024")
    parser.add_argument("--doc-type",      type=str,  default=None, help="文档类型：标准 / 法律")
    ...
    filter_meta = {}
    if args.doc:      filter_meta["doc_id"]   = args.doc
    if args.doc_type: filter_meta["doc_type"] = args.doc_type
