...
class QueryRequest(BaseModel):
    question:   str           = Field(...,  example="电动自行车的最高设计车速是多少")
    doc_id:     Optional[str] = Field(None, example="GB17761-2024")
    doc_type:   Optional[str] = Field(None, example="标准")
...
def _build_source(item: dict) -> str:
    ...
    s = item.get("doc_title", "") or item.get("doc_id", "")
...
def _filter_meta(req: QueryRequest) -> Optional[dict]:
    fm = {}
    if req.doc_id:   fm["doc_id"]   = req.doc_id
    if req.doc_type: fm["doc_type"] = req.doc_type
    return fm or None
