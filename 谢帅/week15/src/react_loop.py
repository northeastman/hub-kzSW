"""
通用 ReAct 循环引擎（代码审查 subagent 项目用）

教学重点：
  1. ReAct = Reason + Act：LLM 生成 Thought(推理) → Action(选工具) → Action Input(参数)，
     runner 执行工具得 Observation，再喂回 LLM 继续，直到 Final Answer
  2. 主 agent 和 reviewer subagent 都是 ReAct 循环——区别只在「有哪些工具」：
     主 agent 有 scan_directory + dispatch_reviewers，reviewer 只有 read_file
...
工具执行后会得到 Observation。多轮调用直到能给出完整结论，最后用：
Thought: 我已收集足够信息
Final Answer: 综合结论（带来源要点）
...
class ReActLoop:
    """通用 ReAct 循环。主 agent / reviewer subagent 各自实例化一个。"""
    ...
    def run(self, question: str, on_step: Callable = None,
        ...
        shared_state: 共享状态 dict（主 agent 派发 reviewer 时往里塞 reviewer trace）。
        ...
        for step_idx in range(self.max_steps):
            ...
            #    "主 agent 决定派发 / reviewer 决定读文件" 的决策，不用等工具返回 ──
            step["final"] = False
            if on_step: on_step(step)

            # 执行工具（可能很慢，如 dispatch_reviewers 要等所有子 agent 跑完）
    ...
    def _parse(self, text: str) -> tuple[str, str, str]:
        ...
        （LLM 拿到审查结果后常直接写报告、不带 Final Answer 前缀）。"""
        thought = ""
        ...
        if text.strip():
            return thought or "综合分析结果给出结论", "Final Answer", text.strip()
        return thought, "", ""
    ...
    def _exec_tool(self, action: str, action_input: str, shared_state: dict) -> str:
        ...
        try:
            # 工具可能需要 shared_state（dispatch_reviewers 用）
...
# ── 自测：单工具 ReAct 跑通（用 file_tools 的 scan_directory）────────────────
if __name__ == "__main__":
    import logging as _l, os
    _l.basicConfig(level=_l.WARNING)
    from file_tools import scan_directory, format_scan_result

    def scan_tool(path, **_):
        return format_scan_result(scan_directory(path))

    loop = ReActLoop("test", tools={"scan_directory": (scan_tool, "扫描目录列出代码文件，参数是目录路径")},
                     max_steps=3)
    test_dir = os.path.dirname(os.path.abspath(__file__))
    r = loop.run(f"扫描 {test_dir} 目录")
    print(f"\n答案: {r['final_answer'][:200]}")
    print(f"trace {len(r['trace'])} 步:")
    for s in r["trace"]:
        print(f"  [{s['idx']}] {s['action']}({(s.get('action_input') or '')[:30]}) → {(s.get('observation') or '')[:50]}")
