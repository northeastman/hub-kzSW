...
用法:
    python make_flashcard.py <data.json>                  # 传 JSON 文件路径
    python make_flashcard.py '<inline-json>'              # 直接传 JSON 字符串（tool-use 场景）
    python make_flashcard.py <data> -o output.html        # 指定输出路径

data 参数既可是 JSON 文件路径，也可是内联 JSON 字符串（供 LLM 现场生成数据传入）。
若为内联 JSON 且含 word 字段，会自动归档一份到 skill 的 data/<word>.json 以便复用。
...
def load_data(raw: str) -> dict:
    """接受 JSON 文件路径或内联 JSON 字符串，返回数据 dict。"""
    p = Path(raw)
    try:
        if p.exists():
            return json.loads(p.read_text(encoding="utf-8"))
    except OSError:
        pass  # 路径过长等情况：按内联 JSON 处理
    return json.loads(raw)


def archive_json(data: dict) -> None:
    """把内联生成的数据归档到 skill 的 data/<word>.json，便于复用与回顾。"""
    word = str(data.get("word", "")).strip().lower()
    if not word:
        return
    data_dir = Path(__file__).resolve().parents[1] / "data"
    data_dir.mkdir(exist_ok=True)
    (data_dir / f"{word}.json").write_text(
        json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")


def main():
    parser = argparse.ArgumentParser(description="生成英语单词 Flash Card HTML")
    parser.add_argument("data", help="JSON 数据文件路径，或内联 JSON 字符串")
    ...
    if "word" not in data:
        print("数据解析失败：缺少 word 字段")
        sys.exit(1)

    # 内联 JSON（非现成文件）才归档，避免重复写已有文件
