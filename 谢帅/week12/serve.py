...
async def _stream_react(question: str, max_steps: int, mode: str, session_id: str = "default"):
    ...
    if mode == "manual":
        ...
        def _make_gen():
            return react_run(question, max_steps=max_steps)
    elif mode == "memory":
        from session_memory import run_with_memory
        def _make_gen():
            return run_with_memory(question, session_id=session_id, max_steps=max_steps)
    else:
...
async def query_memory(req: QueryRequest):
    return StreamingResponse(
        _stream_react(req.question, req.max_steps, "memory", req.session_id),
        media_type="text/event-stream",
        headers={"Cache-Control": "no-cache", "X-Accel-Buffering": "no"},
    )
...
@app.get("/health")
async def health():
    return {"status": "ok", "model": os.getenv("AGENT_MODEL", "qwen-plus")}
