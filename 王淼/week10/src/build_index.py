...
Embedding 方案：
  1. 阿里云 DashScope text-embedding-v3（默认，但需要 API Key 且有免费额度限制）
  2. 本地 BGE-small-zh-v1.5（设置 USE_LOCAL_EMBED=True 使用）

向量库：FAISS（IndexFlatIP，内积 = 归一化后的余弦相似度）

依赖：
  pip install faiss-cpu openai numpy sentence-transformers
"""
...
USE_LOCAL_EMBED = True                # True=使用本地 BGE 模型，False=使用 DashScope API
BGE_MODEL_PATH  = MODELS_DIR / "bge-small-zh-v1.5"

EMBED_MODEL     = "text-embedding-v3"
EMBED_DIM       = 512                 # BGE-small-zh-v1.5 维度为 512；DashScope 可设为 1024/768/512
...
def get_client():
    ...
    if not api_key:
        raise EnvironmentError(
            ...
            "  或设置 USE_LOCAL_EMBED=True 使用本地 BGE 模型"
        )
...
def embed_texts(client, texts: list[str], show_progress: bool = True) -> np.ndarray:
    """
    批量计算 embedding。
    返回 shape=(N, EMBED_DIM) 的 float32 数组，已 L2 归一化。
    """
    if USE_LOCAL_EMBED:
        return embed_texts_local(texts, show_progress)
    else:
        return embed_texts_dashscope(client, texts, show_progress)


def embed_texts_local(texts: list[str], show_progress: bool = True) -> np.ndarray:
    """使用本地 BGE-small-zh-v1.5 模型计算 embedding"""
...
def embed_texts_dashscope(client, texts: list[str], show_progress: bool = True) -> np.ndarray:
    """使用 DashScope API 计算 embedding"""
    all_embeddings = []
...
def build_faiss_index(chunks: list[dict], client=None):
    """
    构建 FAISS 向量索引。
    ...
    logger.info(f"构建 FAISS 索引，维度={actual_dim}...")
...
def build_chroma_index(chunks: list[dict], client=None):
    """
    ChromaDB 版本（可选）。
