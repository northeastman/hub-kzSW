...
EMBED_DIM       = 512                 # BGE-small-zh-v1.5 维度为 512
...
USE_RERANK_BY_DEFAULT = False  # 默认关闭 rerank（需要下载额外模型，网络受限环境建议关闭）

SYSTEM_PROMPT = """你是一个专业的 AI 行业分析助手，专门回答关于中国 AI 大模型产业发展的问题。

回答规则：
1. 只根据【参考资料】中的内容回答，不得引用或编造资料外的数据
2. 若参考资料不足以支撑回答，直接说"根据提供的资料无法回答此问题"
3. 引用具体数据时，在句末标注来源编号，如：市场规模达1000亿元[1]
4. 数字要精确，不得四舍五入或模糊表达
5. 回答简洁，重点突出，避免无关废话"""
...
class VectorStore:
    def __init__(self, client=None):
        ...
        with tempfile.NamedTemporaryFile(suffix='.bin', delete=False) as tmp:
            tmp_path = tmp.name
        shutil.copy2(str(INDEX_PATH), tmp_path)
...
class RAGPipeline:
    def __init__(
        ...
        self.client        = None
        self.vec_store     = VectorStore(None)
    ...
    def query(
        ...
        # ① 查询改写（可选）
        retrieval_query = rewrite_query(question, self._get_client()) if self.use_qr else question
        ...
        answer = call_llm(question, context, self._get_client())
...
def main():
    ...
    parser.add_argument("--rerank",        action="store_true", help="开启 Rerank（需要下载额外模型）")
