...
任务：根据"好"字在文本中的位置进行分类（1-5类）
      "好"在第1位→类别0，第2位→类别1，...，第5位→类别4
      如果文本前5个字符不含"好"→类别0（默认）
模型：Embedding → RNN → MaxPool → BN → Dropout → Linear → Softmax
...
class KeywordRNN(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.fc(pooled)               # (B, NUM_CLASSES)  CrossEntropyLoss内部会做softmax
        return out


# ─── 5. 训练与评估 ──────────────────────────────────────────
def get_position_label(sent):
    """
    根据"好"字在文本中的位置返回标签（0-4）
    "好"在第1位→0，第2位→1，第3位→2，第4位→3，第5位→4
    如果前5个字符中没有"好"，返回0（默认类别）
    """
...
def train():
    ...
    labeled_data = []
    for sent, _ in data:
        label = get_position_label(sent)
        labeled_data.append((sent, label))
    
    print(f"  样本数：{len(labeled_data)}，词表大小：{len(vocab)}")

    split      = int(len(labeled_data) * TRAIN_RATIO)
    ...
    with torch.no_grad():
        for sent in test_sents:
            ...
            prob_value = prob[0, pred_class].item()
            ...
            print(f"  [预测:类别{pred_class+1}(概率{prob_value:.2f}), 好在第{hao_pos if hao_pos > 0 else '未'}位]  {sent}")
