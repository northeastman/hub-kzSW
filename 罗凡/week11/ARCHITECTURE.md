...
> 项目聚焦"三种工具调用方式对比"，多轮 ReAct 循环（用户提问→LLM 调工具→执行→回填→模型根据结果决定继续调工具还是回答→循环直到信息足够）。MAX_TURNS=10 防止无限循环和成本失控。
...
三种方式的 LLM 多轮 ReAct 循环代码**几乎相同**（都是 `for turn in range(1, MAX_TURNS+1): create(tools=) → 无 tool_calls 则 break → 执行 → role=tool 回填 → 继续循环`），差异只在"工具定义从哪来"和"tool_call 怎么执行"——这是对比的教学核心。
...
4. **多轮 ReAct 能力**：茅台 vs 五粮液对比时，模型可能需要先查一家再查另一家，或先用 list_companies 确认再 search。多轮 ReAct 循环支持这种"先查 A 再根据 A 的结果查 B"的链式依赖；MAX_TURNS=10 作为安全上限防止成本失控。
