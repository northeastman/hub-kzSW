...
与其它方式的关系：
  本文件的 LLM 多轮 ReAct 循环代码，和 mode_mcp/run_mcp.py、mode_cli/run_cli.py 几乎一样，
...
SYSTEM_PROMPT = (
    ...
    "涉及天气时调用 get_weather。"
    "你可以多次调用工具，逐步收集信息——每次根据上一轮工具返回的结果，"
    "决定是否需要继续查询更多数据。每次可以并行调用多个工具。"
    "当你收集到足够信息后，直接给出最终回答，不要再调工具。"
)


def run(client, model: str, question: str, verbose: bool = True) -> dict:
    """
    多轮 ReAct 循环：提问 → 模型输出 tool_call → 执行 → 回填 →
    模型根据结果决定继续调工具还是给出最终回答 → 循环直到模型停止调工具。
    返回 {answer, tool_calls, elapsed, turns} 用于对比器汇总。
    """
    messages = [
    ...
    # 【教学时刻 3】：多轮 ReAct 循环 — 模型可多次调用工具，逐步收集信息
    for turn in range(1, MAX_TURNS + 1):
        ...
        # ReAct 关键：如果没有 tool_calls，说明模型认为信息足够，给出最终回答
        ...
        # for-else：循环没被 break（达到了 MAX_TURNS 上限）
        if verbose:
            print(f"  ⚠ 已达到最大轮次限制（{MAX_TURNS}），强制结束")

    answer = msg.content or ""
    if not answer and turn >= MAX_TURNS:
        answer = f"[已达到最大轮次限制 {MAX_TURNS}，但仍未获得最终回答]"

    elapsed = time.time() - t0
    if verbose:
        print(f"\n  → [llm] 最终回答（{elapsed:.1f}s，共 {turn} 轮）")
...
DEMO_QUESTIONS = [
    ...
    "先查看知识库收录了哪些公司，然后从中选一家检索其2023年的研发投入情况。",  # 多轮 ReAct 链式调用演示
]
