...
教学重点：
  ...
  2. 主 agent 和 subagent 都是 ReAct 循环——区别只在「有哪些工具 / 系统提示是什么」：
     主 agent 有 web_search + dispatch_subagents + recommend_next_figure，
     subagent 只有 web_search
...
class ReActLoop:
    ...
    def run(self, question: str, on_step: Callable = None,
        ...
        for step_idx in range(self.max_steps):
            ...
            # 续写历史（observation 截短，避免 history 越滚越大拖垮后续每步的 token）
            history += llm_out + f"Observation: {observation[:800]}\n"
        else:
            # 超过 max_steps：用一次 LLM 调用把已收集内容总结成干净的 Final Answer，
            # 而非直接 dump 原始 observation（避免"（已达最大步数）+原始片段"污染下游合成）
            try:
                final_answer = llm_chat(
                    "你正在收尾。把对话中已收集的信息整理成简洁结论，"
                    "不要提'最大步数'或'收尾'等字样，直接给结论。",
                    history + "\n请把以上已收集的信息整理成简洁结论：",
                    temperature=0.0, max_tokens=512)
            except Exception as e:
                logger.warning(f"收尾总结失败，回退原始observation: {e}")
                final_answer = (self.trace[-1].get("observation", "") or "")[:500]
            step = {"idx": self.max_steps, "agent": self.agent_name,
                    "thought": "达到步数上限，总结已收集内容", "action": "Final Answer",
    ...
    def _parse(self, text: str) -> tuple[str, str, str]:
        ...
        if text.strip():
            return thought or "综合素材写出文章", "Final Answer", text.strip()
        return thought, "", ""
    ...
    def _exec_tool(self, action: str, action_input: str, shared_state: dict) -> str:
        ...
        try:
            # 工具可能需要 shared_state（dispatch_subagents / recommend_next_figure 用）
