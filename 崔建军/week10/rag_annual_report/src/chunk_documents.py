...
def chunk_semantic(
    ...
    def flush(buf: list[dict]) -> dict | None:
        ...
        # 记录所有涉及的页码范围
        page_nums = sorted({b["page_num"] for b in buf})
        page_num = page_nums[0] if page_nums else -1
        # 如果跨越多个页面，记录范围
        if len(page_nums) > 1:
            page_num = f"{page_nums[0]}-{page_nums[-1]}"
        meta = {
            "page_num":   page_num,
            "section":    buf[0]["section_path"] if buf[0]["section_path"] else [],
...
def build_chunk_id(source_file: str, idx: int) -> str:
    return f"{source_file.replace('.json', '')}_{idx:05d}"
...
def process_file(parsed_path: Path, strategy: str = STRATEGY):
    ...
    for idx, chunk in enumerate(raw_chunks):
        chunk_id = build_chunk_id(parsed_path.name, idx)
