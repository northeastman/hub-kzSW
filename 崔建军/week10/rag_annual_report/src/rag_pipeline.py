...
流程：
  (可选) 查询改写
        ↓
  向量检索（本地 bge-small-zh-v1.5 + FAISS）
        +
...
使用方式：
  ...
  python rag_pipeline.py --query "什么是神经网络"
...
TOP_K_RETRIEVE  = 15    # 初始召回数（增大以提高召回率，再通过 rerank 精排）
TOP_K_RERANK    = 5     # Rerank 后保留数（给 LLM）
SCORE_THRESHOLD = 0.30  # 最高得分低于此值时触发拒绝回答（归一化余弦相似度，提高阈值减少低相关内容）

SYSTEM_PROMPT = """你是一个专业的深度学习课程笔记助手，专门回答关于吴恩达深度学习课程的问题。

回答规则：
1. 只根据【参考资料】中的内容回答，不得引用或编造资料外的知识
2. 若参考资料不足以支撑回答，直接说"根据提供的资料无法回答此问题"
3. 引用具体内容时，在句末标注来源编号，如：神经网络的结构包括输入层、隐藏层和输出层[1]
4. 回答简洁，重点突出，避免无关废话
5. 优先使用参考资料中的原文表述，保证准确性
6. 对于概念性问题，提供清晰的定义和核心要点
7. 对于比较类问题，分点列出各方法的优缺点
8. 所有关键信息必须标注来源编号"""


# ── DashScope 客户端（仅用于 LLM） ────────────────────────────────────────────
...
def rerank(query: str, candidates: list[dict], top_k: int = TOP_K_RERANK) -> list[dict]:
    ...
    若模型未下载，自动从 HuggingFace 下载。
    若 sentence-transformers 未安装，自动降级为直接截断。
    """
    ...
    try:
        ...
        if not model_path.exists():
            logger.info(f"Reranker 模型未在本地，正在从 HuggingFace 下载 {model_name}...")
            model_path.mkdir(parents=True, exist_ok=True)
            import shutil
            from huggingface_hub import snapshot_download
            downloaded_path = snapshot_download(repo_id=model_name, local_dir=str(model_path), local_dir_use_symlinks=False)
        ...
        logger.info(f"Rerank 完成，原始 {len(candidates)} 条，精排后保留 {top_k} 条")

    except ImportError as e:
        logger.warning(f"sentence-transformers 未安装，跳过 Rerank: {e}")
        ...
        messages=[
            {
                "role": "system",
                "content": (
                    "你是检索查询优化专家。将用户的问题改写为更适合从深度学习课程笔记中检索信息的精确查询语句。"
                    "规则：\n"
                    "1. 保留核心概念（神经网络、卷积、循环网络、损失函数、梯度下降、激活函数等）\n"
                    "2. 扩展相关关键词，如将'神经网络'扩展为'神经网络结构组成前向传播反向传播'\n"
                    "3. 将自然语言问题转换为检索式关键词组合\n"
                    "4. 对于比较类问题，列出所有相关概念\n"
                    "5. 不要超过80字\n"
                    "6. 直接输出改写后的查询语句，不要解释"
                ),
...
def build_context(retrieved: list[dict]) -> tuple[str, list[dict]]:
    ...
    for i, item in enumerate(retrieved, 1):
        ...
        label = f"[{i}] 深度学习课程笔记"
        if section:
            # section 可能是列表或字符串，统一处理
            if isinstance(section, list):
                label += f" · {' > '.join(section)}"
            else:
                label += f" · {section}"
        if page and page != -1:
            label += f" · 第{page}页"
...
def main():
    parser = argparse.ArgumentParser(description="深度学习课程笔记 RAG 问答（原生版）")
