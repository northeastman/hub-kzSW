...
def get_location(city: str) -> str:
    """
    查询指定城市的地理位置信息（经纬度、所属国家、省份等）。

    Args:
        city: 城市名称，支持中文，例如 "宁德"、"北京"

    Returns:
        包含城市名、国家、省份、经纬度、人口等信息的文字描述
    """
    with httpx.Client(timeout=10.0) as client:
        ...
        loc = max(results, key=_rank_result)
        lat = loc["latitude"]
        lon = loc["longitude"]
        ...
        lines = [
            ...
            f"人口：{population:,}" if population else "人口：未知",
            f"行政级别：{feature_code}",
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", choices=["location", "weather"], default="weather")
    args = parser.parse_args()
    if args.mode == "location":
        print(get_location(args.city))
    else:
