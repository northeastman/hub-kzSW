...
接口：
  POST /query/manual  - 手写版 ReAct，流式返回每步（支持多轮 history）
  POST /query/fc      - Function Calling 版，流式返回每步（支持多轮 history）
  GET  /health        - 健康检查
...
class QueryRequest(BaseModel):
    ...
    history:   Optional[List[HistoryMessage]] = Field(
        ...
        description="多轮对话历史（仅含此前 user/assistant 问答，不含工具中间过程）",
    )
...
async def _stream_react(
    ...
    def _worker():
        try:
            for step_data in react_run(question, max_steps=max_steps, history=hist):
