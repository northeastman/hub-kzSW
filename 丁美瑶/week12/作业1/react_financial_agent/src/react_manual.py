...
- 这是多轮对话：若用户问题引用上文（如「那茅台呢？」「换五粮液呢？」），
  必须结合历史对话补全完整意图后再行动，不要反问用户
"""
...
def run_and_print(question: str, max_steps: int = 10, history: Optional[List[Dict]] = None) -> Optional[str]:
    """执行一轮问答并打印；返回 Final Answer 文本（供多轮历史追加）。"""
    print(f"\n{'='*60}")
    print(f"问题: {question}")
...
def run_interactive(max_steps: int = 10):
    """交互式多轮对话：输入问题连续追问，输入 clear 清空历史，exit 退出。"""
    print(f"\n多轮对话模式（手写Prompt解析 | 模型: {MODEL}）")
    print("输入问题开始对话；输入 clear 清空历史；输入 exit / quit 退出\n")
    history: List[Dict] = []
    ...
    while True:
        ...
        answer = run_and_print(question, max_steps=max_steps, history=history)
        if answer:
