...
triggers: [闪卡, flash card, 单词卡, 英语单词]
---
...
2. **生成 JSON 数据**：自己写出该单词的学习数据，字段如下，用 write_file 工具保存到 `skills/flash-card/data/<word>.json`：
   - `word`：单词
...
3. **生成 HTML**：用 shell_exec 运行脚本：
   ```bash
   python skills/flash-card/scripts/make_flashcard.py skills/flash-card/data/<word>.json
   ```
...
- 原始 JSON 数据集中存放在 `skills/flash-card/data/` 目录，方便复用与回顾。
- **必须使用 write_file 工具写入 JSON 数据，然后用 shell_exec 运行脚本生成 HTML。**
