...
def chat_from_messages(messages: list[dict], max_steps: int = 10) -> Generator[dict, None, None]:
    """
    基于已有的 messages 列表继续 ReAct 循环（支持多轮对话）

    messages 应已包含 system prompt + 历史对话 + 当前用户问题。
    每轮循环追加 assistant 消息和 observation 到 messages 中。
...
def run(question: str, max_steps: int = 10, verbose: bool = True) -> Generator[dict, None, None]:
    """
    便捷封装：从零开始执行单次 ReAct 问答（向后兼容）

    内部创建 messages = [system, user]，然后调用 chat_from_messages()。
    """
    messages = [
